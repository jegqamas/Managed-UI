...
namespace ManagedUI
{
    ...
    [MIRInfo("Themes", "dmi.themes")]
    [MIRResourcesInfo("DMI_Name_Themes")]
    class DMI_Themes : DMI
    {
        public override void OnView()
        {
            ChildItems.Clear();
            // Get all themes
            string file_directory = Path.Combine(Path.GetDirectoryName(Environment.GetCommandLineArgs()[0]), "Themes");
            Directory.CreateDirectory(file_directory);
            string[] files = Directory.GetFiles(file_directory, "*.mt", SearchOption.TopDirectoryOnly);
            foreach (string file in files)
            {
                ...
                chld.CommandID = "tabcontrol.load.theme";
